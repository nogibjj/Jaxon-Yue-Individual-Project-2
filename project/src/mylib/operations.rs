...
pub type WagesData = (String, f64, f64, f64, f64);

// READ: Get data by country
pub fn read_wages_data_by_country(
    conn: &Connection,
    country_name: &str,
) -> Result<WagesData, Box<dyn Error>> {
    let mut stmt = conn.prepare(
        "SELECT country, year_2000, year_2010, year_2020, year_2022 FROM wages WHERE country=?",
    )?;
...
pub fn update_wages_data(
    ...
    println!(
        "Wages data for {} successfully updated to: {}, {}, {}, {}",
        country, year_2000, year_2010, year_2020, year_2022
    );
    Ok(())
}
